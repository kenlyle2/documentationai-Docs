...
Single-site WordPress. WooCommerce + WP Subscriptions handle all financial transactions: new subscriptions, upgrades, cancellations, and order history. Stripe is the payment processor. Users manage billing via the standard WooCommerce `/my-account` page.
...
WordPress users on postglider.com are created automatically by WooCommerce at checkout — every paying subscriber has a WP account.
...
sequenceDiagram
    ...
    Note over App,WP: Pushes gallery token + Supabase anon key to subsite
```
