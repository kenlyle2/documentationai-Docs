...
| X / Twitter | 30 Days Agent (forked `last30days-skill`) | 🔜 planned |
| TikTok | 30 Days Agent (forked `last30days-skill`) | 🔜 planned |
| LinkedIn | Deferred — limited scraper support | ⏸ deferred |

**X and TikTok — 30 Days Agent (coming)**

X and TikTok engagement will be pulled by a new **30 Days Agent** in `postglider-auto` that drives
a forked copy of [`mvanhorn/last30days-skill`](https://github.com/mvanhorn/last30days-skill). It
writes to the same `post_engagement` table and schema, so Butler, Weekly Architect, and the
Autonomous Orchestrator pick up X/TikTok data automatically once it ships.

Two new env vars will be required on Cloud Run (not Vercel):
