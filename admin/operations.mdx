...
(you) to do something (an OAuth click, a Loops "Publish" button) that no API can do for us.
...
**Publish**. Loops' API can build and edit a workflow's graph and content, but activating it
(`Draft` → live) has no exposed endpoint — confirmed by inspecting `updateWorkflowProperties`'s
full schema, which has no `status` field. Until that click happens, the workflow sits built and
tested but not actually sending.
