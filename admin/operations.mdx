...
### Gaps found doing this review — not yet fixed, worth knowing about

Writing the plain-English version above is what surfaced these — they're real, not hypothetical:

1. **The billing test isn't actually testing the customer billing path.** As noted above, the
   account it uses is an admin account, which bypasses the normal charge/limit logic entirely
   (confirmed in the live database function, not assumed). Until this is fixed with a non-admin
   test account, **the actual credit-charging code every paying customer hits has zero automated
   coverage.** This is the single most important gap this review found.
2. **A job that fails to even start reports nothing.** The daily email is built from what each
   job successfully tells the database when it finishes. If a job can't start at all (a
   permissions problem, Google having an outage), it never gets that far — so silence from a
   scheduled job isn't proof everything's fine, it just means nothing was heard from. Worth a
   periodic manual glance at Cloud Scheduler's own dashboard, not just trusting the inbox.
3. **No fast "is the site even up" check exists separately from the heavy 4pm test.** The 4pm
   run takes 10-15 minutes end-to-end; if the site were fully down, we'd only find out at 4pm (or
   whenever someone happens to visit it) rather than within a minute or two of it happening.
4. **Social account connection health (expiring tokens) isn't on any schedule.** A script for
   this already exists (`social-03-token-health.cjs`) but currently only runs when a person
   remembers to run it by hand.
5. **The fast, free correctness checks (`vitest` — pure math tests for billing/batch-size
   formulas) don't run on any schedule either**, only when someone runs them locally. These are
   the cheapest possible tests to run (no browser, no real API calls, seconds not minutes) and
   currently get the least automated attention of anything in the suite.

None of these are fixed yet — flagging them here so they don't quietly disappear, the same reason
the Canva-watermark gap sat unfound for 8 months.
...
| `postglider-auto-test-runner-digest` | `send-test-suite-digest.cjs` | `30 17 * * *` | Not a test itself — reads everything the other jobs reported to `test_suite_runs` since the last run and fires the Loops digest (below). Scheduled last, after the 16:00 UTC quality check finishes. |
...
| `postglider-auto-test-runner-billing` | `billing-02-charge-atomic.cjs` | `0 15 * * 1,3,5` | Real `charge_credits_atomic` RPC contract: accumulation, cycle-rollover, pricing-formula coherence, idempotency. **Its very first scheduled run (2026-09-11) surfaced 2 failures that traced to a test-fixture issue, not a real bug** — see "Known live findings" below. Needs a non-admin fixture account. |
