...
2026-09-11) with its own `entrypoint.vitest-runner.sh` mirroring the same reporting step. **This
image does NOT auto-pick-up new commits** — rebuild it manually whenever a vitest test file or
the code it tests changes:
```bash
cd postglider-auto
gcloud builds submit --config /tmp/cloudbuild-vitest-runner.yaml --project=postglider-prod .
gcloud run jobs update postglider-auto-test-runner-vitest --region=us-central1 --image=us-central1-docker.pkg.dev/postglider-prod/cloud-run-source-deploy/postglider-auto-vitest-runner:<next-tag>
```
(the `cloudbuild.yaml` referenced there just runs `docker build -f Dockerfile.vitest-runner -t ... .` — recreate it if not present locally.) No push-triggered auto-rebuild is wired yet.
