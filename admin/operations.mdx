...
Run these in the **Supabase SQL Editor** (`qzyvfuoyfrfosdtbczkc`).
...
**Deploy Next.js app:** Push to `main` — Vercel deploys automatically on every push to `main`.
...
```bash
curl -X DELETE \
  https://qzyvfuoyfrfosdtbczkc.supabase.co/auth/v1/admin/users/<uuid> \
...
<Steps>
  <Step title="Verify WP subsite is provisioned" icon="check" title-type="p">
  ...
  <Step title="Confirm gallery token exists" icon="key" title-type="p">
    ```sql
...
| `FAL_KEY` | Image generation (Ideogram v3, birefnet) |
