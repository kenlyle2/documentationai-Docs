...
## Automated Test Suite — Scheduled Cloud Run Jobs + Daily Digest

Built 2026-09-11 after a real production embarrassment: a customer's own uploaded flyer (with his
own Canva watermark already on it) got cropped wrong AND double-branded with our own wordmark on
top, live on his Instagram, with zero automated coverage catching it — "That testing gap 8 months
into the project is an absolute disgrace." This section is the actual, provable answer: what runs
on a schedule today, how results reach a human inbox, and exactly what still needs a real person
(you) to do something (an OAuth click, clicking "Start" on a Loops workflow) that no API can do
for us.

### Plain-English summary — what's tested, how often, and what isn't yet

No jargon version, for anyone who wants to know "is this actually working" without reading Cloud
Run job specs. Everything below runs automatically, unattended, every single day it's scheduled
for — nobody has to remember to click anything.

**Every day:**
- **Noon UTC** — checks that our accounts with Google (Gemini), OpenAI, Anthropic (Claude), and
  Replicate are all still valid and none of them have been shut off, revoked, or are erroring.
  This doesn't check dollar balances (none of these four companies actually expose that for a
  normal account — they all bill our card monthly, there's no "prepaid balance" to check), but it
  does catch "this vendor stopped working" the moment it happens, not days later.
- **1pm UTC** — pretends to be a brand-new user clicking through the onboarding checklist on the
  dashboard, and separately does some deliberately careless things to a real test profile (maxes
  out sliders, deletes and re-adds things) to make sure the app doesn't break under sloppy real
  human use.
- **4pm UTC** — the big one. Generates a real post, schedules it, actually publishes it live to
  real Facebook and Instagram test accounts, then runs our real AI defect-checker against the
  actual published image. One of the three posts in every run **uses the exact photo from Mike's
  incident** as a permanent fixture — this is the test that would have caught the original bug
  that started all of this.
- **5:30pm UTC** — not a test itself. Collects everything the day's tests reported and emails you
  one summary, worst news first ("Urgent" → "Prioritized" → "Notices"), via the new Loops
  automation.

**Three times a week (alternating, so something real runs almost every day):**
- **Mon/Wed/Fri, 3pm UTC** — checks the credit/billing math directly against the database
  (does charging credits actually add up correctly, does the monthly reset happen on time, are
  duplicate charges blocked). **Known limitation, not yet fixed:** the account this test uses is
  an admin account, which skips the normal billing rules entirely — so right now this test is
  quietly checking the admin path, not the real customer path. See the correction below.
- **Tue/Thu/Sat, 3:30pm UTC** — runs an entire fake signup from scratch: a made-up business signs
  up for a free trial, goes through onboarding, gets a real AI-generated post, and claims their
  account. This is the closest thing to "a stranger tries PostGlider for the first time," done
  automatically, three times a week.

**Once a week (Sunday):**
- **2pm UTC** — a randomized "monkey test": ~300 semi-random real actions across the app (never
  anything destructive like deleting an account or making a real payment), looking for anything
  that breaks under unpredictable use.
- **3pm UTC** — scans for a specific known data-integrity issue (an anonymous trial session that
  got split from a real paid account, orphaning a social connection).

### Gaps found doing this review — all 5 closed same day (2026-09-11)

Writing the plain-English version above is what surfaced these. Ken: "Attack those 5 issues now
in order, please." All fixed within hours — each one turned up a genuine additional bug along
the way, not just the fix originally scoped:

1. ✅ **Fixed — the billing test wasn't testing the customer billing path.** The account it used
   had admin privileges, which bypasses the normal charge/limit logic entirely. Built a dedicated
   non-admin test account. Along the way, found the test's own expected math was ALSO wrong — a
   real pricing change from 7 weeks earlier had never been reflected in it. Both fixed;
   **the real customer billing path now has real, passing automated coverage for the first time.**
2. ✅ **Fixed — and it caught something real on its very first run.** The daily digest now
   checks "did every job that should have run today actually report in," not just "did any
   reported failures happen." The very first time this ran, it found the 4pm daily check's
   scheduled trigger had a permissions problem and had **never successfully fired on its own
   schedule** — every prior "it's working" belief about that job was based on manual test runs,
   not the actual daily cron. Fixed immediately.
3. ✅ **Fixed — a new check runs every 15 minutes** confirming the site and database are both
   reachable, independent of the heavy 10-15-minute daily test.
4. ✅ **Turned out not to be a real gap.** The token-expiry script checked a database table that
   turned out to be completely empty and unused — the social-media connections it was built to
   watch had already moved to a different vendor (RobinReach) that manages its own token refresh
   internally. Retired the script rather than scheduling a check that would always report "all
   clear" without actually checking anything real.
5. ✅ **Fixed — the fast unit-math tests now run daily too.** Getting there surfaced three real
   technical snags (a Node.js version incompatibility, a missing reporting step, a missing
   security-certificate package) — all found and fixed before anything went live, verified by
   actually running it, not assumed to work.

**Bonus, found once a real email actually arrived:** the digest email itself had a real
formatting bug — the daily counts showed correctly, but the detailed summary text underneath each
one showed the raw placeholder text instead of the real content. Root cause: one specific way of
formatting the email content doesn't fill in live data at all, silently. Fixed and verified with
a real test send before turning the automation back on.

### Architecture

All scheduled tests share ONE generic Cloud Run Job image
(`postglider-auto-test-runner`, `us-central1-docker.pkg.dev/postglider-prod/cloud-run-source-deploy/postglider-auto-test-runner`)
behind **one dedicated Cloud Run Job per scheduled script** — not one job with per-invocation
overrides (that design was tried and abandoned, see `pg-admin/CLAUDE.md`). The entrypoint
(`pg-admin/entrypoint.postglider-auto-test-runner.sh`) fetches the target script fresh via
GitHub's raw-content API at execution time — no image rebuild needed when a test script changes,
only when the base Node/Playwright image itself changes.

**⚠ Real trap, already hit once — don't repeat it:** this image has TWO Dockerfiles in the
`pg-admin` repo (`Dockerfile` for an unrelated service, `Dockerfile.postglider-auto-test-runner`
for this one). `gcloud builds submit --tag ... .` silently picks the DEFAULT `Dockerfile`, not the
named one — the current live tag is `:v3`, built correctly via an explicit
`--config` pointing at a `cloudbuild.yaml` with `-f Dockerfile.postglider-auto-test-runner`. The
`:v2` tag exists in the registry and is the WRONG image (a different service entirely) — never
reference it for any job in this family.

Two features added 2026-09-11 to the entrypoint itself:

1. **`EXTRA_FILES`** (job env var, comma-separated repo-relative paths) — fetches sibling files a
   target script `require()`s by relative path, so a genuinely multi-file script doesn't need its
   shared helpers hand-inlined into a second, driftable copy. Example: the onboarding e2e job
   below sets `EXTRA_FILES=scripts/_shared/onboard-flow.cjs,scripts/_shared/testenv.cjs`.
2. **Automatic reporting** — after every run, the entrypoint POSTs the result (job name, tier,
   exit code, source commit, timestamps) to a new `public.test_suite_runs` table, best-effort
   (never fails the job itself over a reporting hiccup). This is what the daily digest (below)
   reads from.

**One job is structurally different — `postglider-auto-test-runner-vitest`.** `vitest` needs the
full `lib/`+`hooks/` source tree and `node_modules`, which can't be fetched-fresh like a single
script — it has its own dedicated image (`Dockerfile.vitest-runner`, `node:22-bookworm-slim` —
Node 20 has a real incompatibility with this repo's jsdom/undici versions, found and fixed
2026-09-11) with its own `entrypoint.vitest-runner.sh` mirroring the same reporting step.

**Auto-rebuild is wired** (2026-09-11) via a dedicated Cloud Build trigger
(`postglider-auto-vitest-runner-rebuild`, on the same GitHub connection the main app's own
push-trigger uses), scoped with `--included-files` to only fire on a push touching `lib/**`,
`hooks/**`, `vitest.config.mts`, `package.json`, `package-lock.json`, `Dockerfile.vitest-runner`,
or `entrypoint.vitest-runner.sh` — not every unrelated commit. It runs
`cloudbuild.vitest-runner.yaml` (checked into the repo, mirrors the main app's own
build→push→deploy pattern): build → push to Artifact Registry tagged `$SHORT_SHA` → `gcloud run
jobs update --image` (Cloud Run Jobs have no `deploy` subcommand the way Services do). **Verified
live, twice**: two real pushes each produced a real build and a real job update to that exact
commit's image tag, confirmed via `gcloud run jobs describe ...  --format='value(...image)'`
matching the push's short SHA both times — no manual step involved.

Manual rebuild (only needed if the trigger itself ever needs bypassing):
```bash
cd postglider-auto
gcloud builds submit --config cloudbuild.vitest-runner.yaml --project=postglider-prod .
```

### Scheduled jobs, by tier

**Critical** (daily — proves the core pipeline and vendor dependencies are alive):

| Job | Script | Schedule (UTC) | What it proves |
|---|---|---|---|
| `postglider-auto-daily-quality-check` | `daily-quality-check.cjs` | `0 16 * * *` | Real generate → schedule → publish (live to Facebook + Instagram) → real production defect-check against the actual published image. Post 3 of every run uses **the customer's actual incident image** as a permanent regression fixture for the crop/watermark bug that started all this. |
| `postglider-auto-test-runner-vendor-balances` | `check-vendor-balances.cjs` | `0 12 * * *` | Gemini, OpenAI, Anthropic, and Replicate API keys are all still valid and answering. Honest limitation: none of these vendors expose a true prepaid-balance API for a standard key (all four are postpaid to a card) — this checks key health (a cheap live call; 401/403 = FAIL, 429/5xx = WARN), not a dollar figure, and says so explicitly in its own output. |
| `postglider-auto-test-runner-heartbeat` | `check-site-heartbeat.cjs` | `*/15 * * * *` | Fast independent uptime + DB-reachability check (gap #3, closed 2026-09-11) — hits the real public `/api/autonomous/status` endpoint, needs zero secrets. Runs every 15 minutes so an outage is caught in minutes, not only whenever the 4pm test happens to run. |
| `postglider-auto-test-runner-vitest` | `vitest run lib hooks` (own dedicated image, `Dockerfile.vitest-runner`) | `0 11 * * *` | The full `vitest` unit suite (gap #5, closed 2026-09-11) — pure billing/batch-size math, no browser, no billed API calls, seconds not minutes. **Unlike every other job, this image bakes in the source tree at build time and does NOT auto-pick-up new commits** — must be manually rebuilt when a test file or the code it tests changes (see "Architecture" below for the command). |
| `postglider-auto-test-runner-digest` | `send-test-suite-digest.cjs` | `30 17 * * *` | Not a test itself — reads everything the other jobs reported to `test_suite_runs` since the last run and fires the Loops digest (below), including any expected job that never reported at all. Scheduled last, after the 16:00 UTC quality check finishes. |

**Important** (Mon/Wed/Fri or Tue/Thu/Sat — real coverage, lower urgency than daily):

| Job | Script | Schedule (UTC) | What it proves |
|---|---|---|---|
| `postglider-auto-test-runner` | `chaos-onboarding-walkthrough.cjs` | `0 13 * * *` (still daily as of this writing — candidate to move to this tier, not yet decided) | Every dashboard onboarding-checklist link navigates somewhere real; deliberate careless-user mutation testing on a real profile. |
| `postglider-auto-test-runner-billing` | `billing-02-charge-atomic.cjs` | `0 15 * * 1,3,5` | Real `charge_credits_atomic` RPC contract: accumulation, cycle-rollover, pricing-formula coherence, idempotency. **Fixed 2026-09-11** — now runs against a dedicated non-admin fixture account (`kenlyle+billingtest@gmail.com`) and asserts the current (not stale) pricing formula. 10/10 pass against the real customer billing path. |
| `postglider-auto-test-runner-onboarding-e2e` | `app-17-COLD-free-trial-e2e.cjs --onboarding-only` | `30 15 * * 2,4,6` | The single most comprehensive onboarding test in the repo: free-trial signup → business enrichment → Instagram step → claim → `/generate` smoke check, against a fake fixture business (not a real customer). |

**Weekly** (unchanged, pre-existing): `postglider-auto-test-runner-fuzzer`
(`chaos-fuzzer.cjs --actions 300`, Sunday) and `postglider-auto-test-runner-detect-splits`
(`detect-split-accounts.cjs`, Sunday).

### Real bug found and fixed getting `app-17` onto this schedule

`app-17-COLD-free-trial-e2e.cjs` failed its first two live runs in this job pattern —
`SUPABASE_SERVICE_ROLE_KEY not set`, despite the secret being correctly injected as an env var.
Root cause: the script destructured only the raw, unresolved `env` dict from its shared
`testenv.cjs` helper and rebuilt `SUPABASE_URL`/`SERVICE_ROLE_KEY` from it directly — bypassing
`testenv.cjs`'s own already-correct fallback to `process.env` for exactly this "no `.env.local`
file in the container" case. Fixed by using `testenv.cjs`'s exported constants directly instead
of re-deriving them. A second, real, different gap surfaced immediately after: the script's
default `staytrue` business fixture needs a workspace photo file that isn't present in the
minimal container (same class of gap `daily-quality-check.cjs` hit for `ref_ken.jpg`, solved
there via a base64 Secret Manager value). Rather than repeat that fix for a scheduled regression
test that doesn't need a specific business's branding, the scheduled job simply uses the
`roccos` fixture instead (`workspacePhoto: null` — no file dependency at all).

### The daily digest — Loops workflow

`send-test-suite-digest.cjs` reads every `test_suite_runs` row not yet digested, buckets them
(PHP-error-level naming): **Urgent** = a critical-tier job failed, **Prioritized** = an
important-tier job failed, **Notices** = everything else (including all passing runs — proof of
life, not just absence of alarm). Fires the `test_suite_digest` Loops event (registered in
`lib/loops.ts`'s `LoopsEventName` union, same pattern as `gemini_quota_exceeded` and the other
existing ops-alert events) with the three summaries as event properties.

The actual email is a Loops workflow, built via the Loops MCP API this session:
**https://app.loops.so/workflows/cmtx2tdul04zk0j0uogxiv4eq** — `EventTrigger` on
`test_suite_digest` (re-eligible, so it fires every day for the same recipient, not just once
ever), one email action rendering Urgent → Prioritized → Notices in that order via
`{event.urgentSummary}` / `{event.prioritizedSummary}` / `{event.noticesSummary}`, all fallback
values set, Guardian-checked clean.

**One step the API cannot do — you have to click it:** open the workflow link above and click
**Start** (the button is labeled "Start" in the Loops UI, not "Publish" — done 2026-09-11). Loops'
API can build and edit a workflow's graph and content, but activating it (`Draft` → live) has no
exposed endpoint — confirmed by inspecting `updateWorkflowProperties`'s full schema, which has no
`status` field.

### Known live findings

**`billing-02-charge-atomic`'s very first scheduled run (2026-09-11, Friday 15:00 UTC) reported
two failures — traced immediately after, and NOT a real regression.** Both failures
(`current_cycle_start` not written back; `monthly_limit` returning `999999` instead of the
formula-derived `585`) turned out to be the same single, correct, intentional cause: the test
account (`kenlyle+2@gmail.com`) has `profiles.is_admin = true`, and `charge_credits_atomic`'s own
"ADMIN BYPASS" branch (`supabase/migrations/20260622002000_charge_credits_atomic_general_anchor.sql`
line ~125 — confirmed against the live, currently-deployed function, not an old migration)
deliberately tracks usage but skips the normal cycle-rollover write-back and always returns the
`999999` sentinel for an admin account, by design. The test itself is exercising a real,
intentional code path — it's just the wrong path for what this test is trying to verify. **Real
follow-up, not yet done:** either point this test at a non-admin fixture account, or have it
detect and explicitly skip/flag the admin-bypass branch instead of asserting against it. Left as
a known false-positive in the digest until fixed — check `research/decisions.md` D-2026-09-11a
for the investigation trail.

**The `daily-quality-check` run that prompted re-verifying this whole suite (2026-09-11) also
found a real gap**: the test account's RobinReach brand has a live Instagram connection but zero
Facebook connection, and the app's local connection-mirror table had never synced even the
Instagram one — the daily check correctly reported FAIL rather than a false green. Fix needs a
real human OAuth click (RobinReach's Magic Connect Link) — cannot be scripted.

### Adding a new scheduled test

1. Confirm the script is genuinely self-contained (Node builtins + npm packages only) OR uses
   `EXTRA_FILES` for real sibling dependencies — never hand-inline a copy of shared logic.
2. `gcloud run jobs create postglider-auto-test-runner-<name> --region=us-central1 --image=...:v3 --service-account=731600875541-compute@developer.gserviceaccount.com --set-env-vars="REPO_OVERRIDE=...,BRANCH_OVERRIDE=...,JOB_NAME=<script-basename>,TIER=<critical|important>[,EXTRA_FILES=...]" --set-secrets="GITHUB_PAT=...,SUPABASE_SERVICE_ROLE_KEY=...[,<other secrets>]" --args="<path-to-script>[,<script args>]"`
   — note: if any env-var VALUE itself contains a comma (like `EXTRA_FILES`), use gcloud's
   custom-delimiter syntax: `--set-env-vars="^;^KEY1=val1;KEY2=val,with,commas"`.
3. `gcloud run jobs add-iam-policy-binding <job> --region=us-central1 --member="serviceAccount:postglider-cron-invoker@postglider-prod.iam.gserviceaccount.com" --role="roles/run.invoker"`
4. `gcloud scheduler jobs create http <job>-schedule --location=us-central1 --schedule="<cron>" --uri=".../jobs/<job>:run" --http-method=POST --oauth-service-account-email="postglider-cron-invoker@postglider-prod.iam.gserviceaccount.com"`
5. **Run it live once manually before trusting the schedule** — `gcloud run jobs execute <job> --region=us-central1`, then check `gcloud logging read 'resource.type="cloud_run_job" AND resource.labels.job_name="<job>"' --freshness=10m`. Every job in this family that skipped this step on first deploy has hit a real, container-specific bug (missing env fallback, missing fixture file, missing secret) that never showed up running locally.
...
## Customer social-account connections (RobinReach)

How a customer connects Instagram/Facebook/etc. to their business profile, end to end, fully
automated as of 2026-08-22 — no admin step of any kind.

1. Each business profile gets its own RobinReach **Brand** (`business_profiles.robinreach_brand_id`),
   created lazily on first use via `ensureRobinReachBrand()` (`lib/actions/socialPublishActions.ts`)
   — one Brand per business profile, never shared.
2. From `/settings/social`, the customer clicks **Add channel**, which opens
   `components/settings/RobinReachConnectModal.tsx`. On open, `getRobinReachConnectLink()` calls
   RobinReach's real `POST /connect_links` API and returns a fresh, Brand-scoped **Magic Connect
   Link** — a real URL (`https://robinreach.com/connect/{token}`) the customer opens to connect
   their accounts directly. No RobinReach account, no login, and no RobinReach branding is shown
   in that flow, so it reads as part of the PostGlider product.
3. RobinReach has **no webhooks** — there's no way for RobinReach to push "this customer just
   connected Instagram" to us. The customer clicks **Check connections** back in the modal (or it
   auto-checks on open), which calls `syncAndListRobinReachAccounts()` — a live `GET
   /social_profiles?brand_id=...` reconciliation that upserts `social_publish_connections` and
   returns the current state. The same reconciliation also runs implicitly whenever
   `/settings/social` loads.

**Before 2026-08-22** this required a manual step: an admin (Ken) had to individually invite each
customer's email into their Brand from RobinReach's own dashboard before they could log in at
all, since RobinReach had no invite API — a `requestRobinReachAccess()` action fired an ops-alert
Loops event asking an admin to go do this. Once RobinReach's Magic Connect Link API was confirmed
working, `getRobinReachConnectLink()` replaced that entire flow — see the workspace's
`research/decisions.md` D-2026-08-22b/c.
...
## Loops email architecture — the rule that prevents duplicate sends (2026-08-22)

`lib/loops.ts`'s `LoopsEventName` union is the registry of every event this app can fire via
`loopsFireEvent()`. Two structurally different delivery mechanisms exist in this codebase for
turning an event into an actual email, and **mixing both for the same event is what caused a real,
live bug** (found and fixed 2026-08-22, `decisions.md` D-2026-08-22j):

1. **Loops-native workflows** — an `EventTrigger` node built in the Loops dashboard/API
   (`GET /v1/workflows`), with its own timers/branches, entirely inside Loops. `lib/loops.ts`'s
   `loopsFireEvent()` is the only code-side call needed; Loops handles the rest.
2. **App-code-driven sends** — either an Upstash Workflow route under `app/api/workflows/*`
   (`triggerWorkflow()`, for multi-step/delayed sequences like trial nurture) or a direct
   `loopsSendTransactional()` call at the event's own call site (for a single immediate email,
   the pattern used for the 2026-08-22 ops-alert/notification emails below).

**Rule: pick exactly one mechanism per event, never both.** The "Trial Nurture Sequence" incident
was `trial_started` wired to a live Loops-native workflow *and* an Upstash-driven
`/api/workflows/trial-nurture` route simultaneously — every trial signup got the Day 0/3/7 emails
twice. Before adding a new event or a new email for an existing one, check `GET /v1/workflows`
for a matching `EventTrigger` (Loops account, not just this codebase) as carefully as you'd check
this codebase's own call sites.

**Current default for new events (established 2026-08-22): direct `loopsSendTransactional()` at
the call site, not a Loops-native workflow.** Keeps sequencing logic in code (reviewable, testable,
git-blame-able) rather than split across two systems. `loopsFireEvent()` is still called
alongside it — its only remaining job is contact-property tagging/segmentation in Loops, not
triggering a send. Transactional template ids live in `lib/config/workflowTemplates.ts`
(`WORKFLOW_TEMPLATES`), one constant per event — never hardcode an id at a call site (the one
exception, documented inline, is `detect-split-accounts.cjs`, which must stay dependency-free to
run standalone as a Cloud Run Job).

**Before firing a new event for the first time, ask:** does an existing Upstash Workflow route or
transactional send already cover this moment under a different trigger? `subscription_upgraded`
looked like a gap in the 2026-08-22 audit but turned out to already be covered by
`/api/workflows/pro-welcome` (triggered at the same call site, for the same tier-change) — building
a dedicated email for the event itself would have been a second, duplicate mechanism for the exact
moment `pro-welcome` already handles. See `decisions.md` D-2026-08-22k for the full reasoning.

---

## Local Partners — Real, Geo-Scoped, Owner-Approved Local-Business Referrals (2026-08-20, opt-in redesign 2026-08-22)
...
**Explicit opt-in gate (2026-08-22, `research/decisions.md` D-2026-08-22i):** discovery only ever
fires when `business_profiles.local_partners_enabled = true` — never inferred, never on by
default. The original 2026-08-20 design correctly avoided a blanket auto-trigger, but discovery
was reachable *only* via the Settings button, which most businesses (especially brand-new
signups, with nothing partner-worthy to name yet) never find on their own. Three ways the flag
gets set:

1. **Onboarding opt-in** — a checkbox on `/onboarding`'s Instagram-handle step: "Do you have
   partners you want to promote via your social channel? Possibly suppliers, secondary geographic
   locations, affiliates, or vendors?" Off by default; feeds `wantsLocalPartners` through
   `POST /api/onboarding/run` → `runOnboardingPipeline()`.
2. **Settings toggle** — `LocalPartnersCard.tsx`'s header gained an on/off switch
   (`PATCH /api/local-partners/toggle`); the card's body (suggest/approve/manual-add) is hidden
   while off.
3. **Delayed reminder** — `app/api/cron/local-partners-reminder` (daily; **Cloud Scheduler job
   not yet created**, see this repo's TASKS.md) finds profiles 30-31 days old, still off, never
   reminded (`local_partners_reminder_sent_at IS NULL`), and fires a `local_partners_reminder`
   Loops event pointing them at My Vibe settings.

Turning the flag on (either path 1 or 2) fires `findLocalPartners()` once immediately — the
owner doesn't have to separately click "Suggest Partners" right after opting in.

### What's built

| Component | Path | Purpose |
|---|---|---|
| **Migration** | `supabase/migrations/20260820040000_local_partners.sql`, `20260822010000_local_partners_opt_in.sql` | `local_partners` table + RLS; `business_profiles.local_partners_enabled`/`local_partners_reminder_sent_at` |
| **Discovery** | `lib/integrations/localPartnerFinder.ts` | Google Places Text Search by category (dry cleaner, food/grocery delivery), geo-filtered to the owner's own `metro_cbsa_code` — the same CBSA-tagging mechanism the Publish Time Optimizer section above uses. Gated on `local_partners_enabled`, callable from onboarding, the Settings toggle, or the existing "Suggest Partners" button. |
| **API** | `app/api/local-partners/route.ts`, `app/api/local-partners/toggle/route.ts`, `app/api/find-local-partners/route.ts` | CRUD + on/off toggle + discovery-trigger, same conventions as `/api/competitors`/`/api/find-competitors` |
| **Reminder cron** | `app/api/cron/local-partners-reminder/route.ts` | Daily; ~30 days post-signup, still off, not yet reminded → fires Loops event, stamps the guard column |
| **Settings UI** | `components/settings/LocalPartnersCard.tsx` | On/off switch in the header → (when on) "Suggest Partners" button → pending candidates with Approve/Reject → Approved Partners list |
...
that same shape directly from ScrapeCreators (`/v1/instagram/profile` or `/v1/facebook/profile` +
paginated posts) instead of scraping a UI. **Status: promoted to a real agent + route —
`lib/autonomous/scrapeCreatorsCompetitiveIntelAgent.ts` (`runScrapeCreatorsCompetitiveIntelAgent()`)
and `POST /api/autonomous/scrapecreators-competitor-intel-agent`.** No Chromium needed, so unlike
the Predis agent above this one runs inside the main `postglider-autonomous` Cloud Run service
rather than a dedicated one. Writes to the same `autonomous.competitor_intel_predis` table as the
Predis pipeline, distinguished by a `source` column (`'predis' | 'scrapecreators'`) — not yet in
the Orchestrator's automatic trigger path (manual/onboarding-triggered call only for now). Full
narrative and real measured costs are in `docs/BUILD-HISTORY.md`'s 2026-06-24 entries.

**Facebook support + cross-channel wrapper (2026-08-22).** Was Instagram-only until this date —
`getFacebookProfile()` (follower count) was the last missing piece after Facebook posts were
wired 2026-08-18. `runScrapeCreatorsCompetitiveIntelAgent()` now takes a real
`platform: 'instagram' | 'facebook'` param, and a new `runCrossChannelCompetitiveIntel()` wrapper
runs both and adds `computePlatformSync()`'s day-of-week flip/alignment classification — the same
mechanism the original ManFlowYoga optimizer work (above) is grounded in. See `research/
decisions.md` D-2026-08-22h for the real report this was built for (Lenny Calisthenics).

**A second consumer, distinct from third-party competitor reports: Layer 3 self-analysis, now a
standard onboarding step (2026-08-22).** `lib/onboarding/seedOwnEngagementSignal.ts` calls this
same agent against the *signing-up business's own* handle (not a competitor's) and upserts the
resulting posts into `post_engagement` — bootstrapping the Publish Time Optimizer's "own
engagement, self-improving" layer (see that section above) from real historical performance
instead of waiting weeks for it to accumulate via `engagement-refresh`. Wired into
`runOnboardingPipeline.ts` as a fire-and-forget step, real cost confirmed well under $0.20
combined for both platforms — cheap enough to run for every onboarding with a known handle, not
gated behind any cadence decision. See `research/decisions.md` D-2026-08-22h.
...
**Third-party competitor report onboarding integration — still not built, design only.** (Distinct
from the Layer 3 self-analysis integration above, which IS built — this paragraph is specifically
about generating a report on a *competitor's* account at signup time.) This still depends on
...
ready-notification, the same async pattern other autonomous agents already use (and the same
fire-and-forget pattern the Layer 3 seed already uses). See `TASKS.md` in `postglider-auto` for
the open onboarding-wiring items.
...
## PostHog Weekly Findings Digest (postglider-auto, 2026-09-08)

Full detail lives in `admin/posthog.mdx` §4 — this entry is here so the cron itself shows up in
the operational cadence inventory, per this doc's own routing convention.

| | |
|---|---|
| **Cron endpoint** | `app/api/cron/posthog-weekly-digest/route.ts` |
| **Schedule** | Cloud Scheduler job `postglider-posthog-weekly-digest`, Mondays 9am UTC |
| **What it does** | Queries PostHog (HogQL) for the trailing 7 days — `$exception` count/top messages, `$rageclick`/`$dead_click` counts, p75 Web Vitals, session-replay volume — emails the summary via Loops (`OPS_POSTHOG_WEEKLY_DIGEST`) to kenlyle@gmail.com + mike@postglider.com |
| **Env vars** | `POSTHOG_PERSONAL_API_KEY`, `POSTHOG_PROJECT_ID` (see Environment Variables Reference below) |
| **Manual trigger** | `curl -H "Authorization: Bearer $CRON_SECRET" https://postglider-autonomous-<project>.us-central1.run.app/api/cron/posthog-weekly-digest` — **sends a real email each time**, see the warning in `posthog.mdx` §4 |
...
| `POSTHOG_PERSONAL_API_KEY` | `posthog-weekly-digest` cron — HogQL query access (`query:read` scope) |
| `POSTHOG_PROJECT_ID` | `posthog-weekly-digest` cron — numeric PostHog project ID |
