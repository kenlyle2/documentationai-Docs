...
### MuAPI relationship — barred for images (2026-07-20)

**MuAPI is barred as a vendor for image generation, period, unless Ken explicitly reopens it.**
Confirmed live 2026-07-20 (decisions.md D-2026-07-20m) that this has *already happened* in code —
`lib/config/imageModels.ts`'s `IMAGE_ROUTING` table has zero `MUAPI` entries; every image route
(`GENERAL`, `GENERAL_COMPLEX`, `PERSON_FOREGROUND`, etc.) is `GOOGLE`, `OPENAI`, or `REPLICATE`.
The paragraphs that used to live here — "we want to like MuAPI," a price comparison, a policy of
"use MuAPI where they're the best option" — described a configuration this app no longer runs and
directly contradicted the current standing order. MuAPI's *video*-generation usage
(`VIDEO_ROUTING`) is untouched and out of scope for this ban — a separate system, not reopened by
this entry.
