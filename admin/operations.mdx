...
Which model backs each tier is a routing decision, not a doc fact — read `lib/config/imageModels.ts`
for the current DRAFT/DEFAULT/PREMIUM → model mapping rather than trusting a table here. DEFAULT
fires automatically on the Make Reel button and the Person in Scene auto-trigger; DRAFT/PREMIUM
are explicit-only.
