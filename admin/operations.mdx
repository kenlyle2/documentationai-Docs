...
| `postglider-auto-test-runner-billing` | `billing-02-charge-atomic.cjs` | `0 15 * * 1,3,5` | Real `charge_credits_atomic` RPC contract: accumulation, cycle-rollover, pricing-formula coherence, idempotency. **Its very first scheduled run (2026-09-11) found two real, live regressions** — see "Known live findings" below. |
...
### Known live findings — real regressions the suite has already caught

**`billing-02-charge-atomic`'s very first scheduled run (2026-09-11, Friday 15:00 UTC) found two
real, live failures**, not test-harness noise:
1. `current_cycle_start` is not being written back on cycle rollover — the test account's value
   (`2026-08-17`) is stale, "before this month."
2. `monthly_limit` from the RPC returns `999999` instead of the formula-derived `585` — looks like
   an "unlimited" sentinel value is in play somewhere for this account, not simply a stale
   hardcoded `750` (the original B3 bug this test was written to catch).

Not yet root-caused or fixed as of this writing — flagged here, and will surface again in the
daily Loops digest once published, so it can't quietly disappear. Check `research/decisions.md`
for the follow-up entry once investigated.
