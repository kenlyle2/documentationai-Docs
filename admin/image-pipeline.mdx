...
```mermaid
flowchart TD
    A[Image uploaded to Supabase Storage] --> B[user_images row inserted]
    B --> C[tagImageAction fires]
    C --> D[Verify image URL is publicly accessible — 5 retries, 3s timeout]
    D --> E[Gemini 1.5 Flash — base64 image + system prompt]
    E --> F{Success?}
    F -- Yes --> G[tags + description returned]
    F -- No --> H[GPT-4o-mini Vision fallback]
    H --> G
...
A separate Gemini call scores each image on marketing quality (1–10 scale + PASS/REJECT). Stored as:
...
Users can re-tag any individual image from the image detail dialog in the vault. This re-runs the Gemini tagging call with a fresh prompt and overwrites the existing tags. Useful when auto-tags missed something obvious or when business terminology has been updated in the profile.
