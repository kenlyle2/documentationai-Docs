...
SearchIQ can only index WordPress content — it cannot call external REST APIs directly. The PostGlider Adapter creates thin **stub posts** inside each client's WordPress subsite. SearchIQ indexes these stubs; actual image files never leave Supabase Storage.
...
Tokens are `pg_gallery_` + 24 random bytes = 48 hex characters (~192 bits of entropy). They are stored in plaintext in `gallery_tokens` — Supabase RLS and service-role-only write access are the security boundary. Clients can revoke instantly if compromised.
...
<Steps>
  <Step title="Install SearchIQ plugin on the subsite" icon="package" title-type="p">
    Network-activate or per-site install via WordPress admin.
  </Step>
  <Step title="Enter the SearchIQ API key" icon="key" title-type="p">
  ...
  <Step title="Trigger an index crawl" icon="search" title-type="p">
