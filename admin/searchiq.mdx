...
For users with untagged images, use the admin endpoint — it re-tags and syncs in one pass:
...
When images are already tagged in Supabase but never reached WordPress (e.g., sync calls failed before the Cloudflare Worker was deployed), call `sync-image` directly for each image:
...
curl -X POST "https://<slug>.mypostglider.website/wp-json/postglider/v1/sync-image" \
  ...
  -d '{
    ...
    "tags": ["tag1", "tag2"],
    "description": "One-sentence AI description."
  }'
```

To get the gallery token and image data for a user:
